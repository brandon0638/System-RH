...
<div class="app-wrap">
    ...
    <div class="main">
        ...
        <div class="content">
            ...
            <div class="metrics">
                ...
                <div class="metric">
                    ...
                    <div class="metric-val"><?= $total_restant ?? 0 ?></div>
                ...
                <div class="metric">
                    <div class="metric-top"><div class="metric-icon mi-blue"><i class="bi bi-calendar3"></i></div></div>
                    <div class="metric-val"><?= $total_demandes ?? 0 ?></div>
                ...
                <div style="padding:1rem 1.25rem;display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1rem">
                    <?php if(!empty($soldes)): ?>
                        ...
                        <div class="solde-card" style="margin:0">
                            ...
                            <div class="solde-bar">
                                <div class="solde-fill <?= ($solde['restant_jours'] / $solde['total_jours']) * 100 < 20 ? 'danger' : '' ?>" style="width: <?= ($solde['total_jours'] > 0 ? ($solde['restant_jours'] / $solde['total_jours']) * 100 : 0) ?>%"></div>
                            </div>
                            <div class="solde-label"><?= $solde['restant_jours'] ?> jours restants · <?= $solde['pris_jours'] ?> pris</div>
                        </div>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <div class="empty">Aucun solde trouvé</div>
                    <?php endif; ?>
                </div>
            </div>
                                    ...
                                    <td>
                                        <?php if($demande['statut'] === 'en_attente'): ?>
                                            <form action="<?= base_url('/employe/annuler/' . $demande['id']) ?>" method="post" style="display:inline">
                                                <?= csrf_field() ?>
                                                <button type="submit" class="btn-sm btn-cancel" onclick="return confirm('Annuler cette demande ?')">
        ...
        <div class="footer-app">
            <i class="bi bi-c-circle"></i> <?= date('Y') ?> <span>TechMada RH</span> — Gestion des congés CI4
        </div>
    </div>
</div>
